defmodule ManaChessOnlineWeb.GameLiveTest do
  ...
  test "lobby exposes the complete release cosmetic catalog", %{conn: conn} do
    ...
    assert has_element?(view, ~s([data-cosmetic-mastery-progress][aria-valuemax="5"]))

    for skin <- ~w(arcane crystal elemental celestial) do
  ...
  test "practice exposes bot difficulties and tutorial starts as a guided lesson", %{conn: conn} do
    ...
    refute has_element?(view, ".mc-board-coordinate-frame.is-tutorial")
    refute has_element?(view, ".mc-tutorial-classic-piece")
    ...
    assert has_element?(view, ".mc-board-coordinate-ranks-left span", "2")
